...
const MainPage = () => {
    ...
    const getCurrentStatusRegister = () => {
        ...
        if (
            startOfficialElitech <= currentDateString &&
            currentDateString <= endOfficialElitech
        ) {
            return `ĐK chính thức: CT ELITECH ${getFormatedDate(
                startOfficialElitech
            )} -> ${getFormatedDate(endOfficialElitech)}`;
        } else if (
            startOfficialStandard <= currentDateString &&
            currentDateString <= endOfficialStandard
        ) {
            return `ĐK chính thức: CT Chuẩn ${getFormatedDate(
                startOfficialStandard
            )} -> ${getFormatedDate(endOfficialStandard)}`;
        } else if (
            startUnofficialElitech <= currentDateString &&
            currentDateString <= endUnofficialElitech
        ) {
            return `ĐK điều chỉnh: CT ELITECH ${getFormatedDate(
                startUnofficialElitech
            )} -> ${getFormatedDate(endUnofficialElitech)}`;
        } else if (
            startUnofficialStandard <= currentDateString &&
            currentDateString <= endUnofficialStandard
        ) {
            return `ĐK điều chỉnh: CT Chuẩn ${getFormatedDate(
                startUnofficialStandard
            )} -> ${getFormatedDate(endUnofficialStandard)}`;
        } else if (startFree <= currentDateString && currentDateString <= endFree) {
            return `ĐK tự do ${getFormatedDate(startFree)} -> ${getFormatedDate(
                endFree
            )}`;
        }
    ...
    const fetchMetadataSemester = async (currentSemester) => {
        ...
        setStartOfficialElitech(
            jsonData[Constant.START_REGISTER_CLASS_OFFICIAL_ELITECH]
        );
        setEndOfficialElitech(
            jsonData[Constant.END_REGISTER_CLASS_OFFICIAL_ELITECH]
        );
        setStartOfficialStandard(
            jsonData[Constant.START_REGISTER_CLASS_OFFICIAL_STANDARD]
        );
        setEndOfficialStandard(
            jsonData[Constant.END_REGISTER_CLASS_OFFICIAL_STANDARD]
        );
        setStartUnofficialElitech(
            jsonData[Constant.START_REGISTER_CLASS_UNOFFICIAL_ELITECH]
        );
        setEndUnofficialElitech(
            jsonData[Constant.END_REGISTER_CLASS_UNOFFICIAL_ELITECH]
        );
        setStartUnofficialStandard(
            jsonData[Constant.START_REGISTER_CLASS_UNOFFICIAL_STANDARD]
        );
        setEndUnofficialStandard(
            jsonData[Constant.END_REGISTER_CLASS_UNOFFICIAL_STANDARD]
        );
